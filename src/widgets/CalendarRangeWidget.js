...
export default class CalendarWidget extends InputWidget {
  ...
  attach(input) {
    ...
    this.settings.onChange = () => {
      console.log('flatpickr onChange',this.settings.manualInputValue);
          ...
          .then((Flatpickr) => {
            ...
            if (this._input) {
              ...
              if (locale && locale.length >= 2 && locale !== 'en') {
                Formio.requireLibrary(`flatpickr-${locale}`,
                  `flatpickr-${locale}`,
                  `${Formio.cdn['flatpickr-formio']}/l10n/flatpickr-${locale}.js`,
                  true);
                Formio.libraryReady(`flatpickr-${locale}`)
                  .then(() => this.initFlatpickr(Flatpickr));
              }
              else {
  ...
  getValue() {
    ...
    if (dates.findIndex(date=>!(date instanceof Date))>-1) {
      return 'Invalid Date';
    }

    return dates.map(date=>{
      return this.getDateValue(date, this.valueFormat, (this.settings.saveAs === 'date'));
    });
  }
  ...
  setValue(value) {
    ...
    if (value && value.length > 0) {
      if (!saveAsText && this.settings.readOnly && !zonesLoading) {
        this.calendar.setDate(value.map(date=>{
        ...
        this.calendar.setDate(value.map(date=>{
          return moment(date, this.valueMomentFormat).toDate();
        }), false);
      }
    }
    else {
  ...
  initFlatpickr(Flatpickr) {
    ...
    this.calendar.altInput.addEventListener('input', (event) => {
      console.log('inputValue altInput input',event.target.value,this.settings.currentValue,this.calendar.selectedDates);
    ...
    this.addEventListener(this.calendar._input, 'blur', (event) => {
      ...
      if (!(isIEBrowser && !relatedTarget) && !this.isCalendarElement(relatedTarget)) {
        ...
        // 范围分割字符
        const rangeSeparator=this.calendar.l10n.rangeSeparator??' to ';
        const dateValue = inputValue ? inputValue.split(rangeSeparator).map(value=>{
          return moment(value.trim(), convertFormatToMoment(this.valueFormat)).toDate();
        }) : [];
