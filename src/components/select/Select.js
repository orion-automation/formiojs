...
export default class SelectComponent extends ListComponent {
  ...
  loadItems(url, search, headers, options, method, body) {
    ...
    if (
      (this.component.searchField || (this.component.data.noco_db_conditions&&this.component.data.noco_db_conditions.length>0&&this.component.data.noco_db_conditions[0].name.length>0))&&
      (minSearch > 0) &&
    ...
    if (this.component.data.noco_db_conditions&&this.component.data.noco_db_conditions.length>0&&this.component.data.noco_db_conditions[0].name.length>0&&search&&search.length>0){
      ...
      this.component.data.noco_db_conditions.forEach((item,index)=>{
        if (index===0 && item.logical_operator==="~not"){
          where+=`(${item.name},${item.operator},${search})`
        } else {
          where+=`${item.logical_operator}(${item.name},${item.operator},${search})`
        }
      });
      query.where=where;
    }
  ...
  attach(element) {
    ...
    if (Choices) {
      ...
      if (this.choices.input && this.choices.input.element){
        this.choices.input.element.addEventListener("input",this.choices._onKeyUp,{passive: true})
      }
    ...
    if (this.component.searchField || (this.component.data.noco_db_conditions&&this.component.data.noco_db_conditions.length>0&&this.component.data.noco_db_conditions[0].name.length>0)) {
      ...
      const debounceTimeout = (this.component.searchField||(this.component.data.noco_db_conditions&&this.component.data.noco_db_conditions.length>0&&this.component.data.noco_db_conditions[0].name.length>0)) && (this.isSelectResource || this.isSelectURL) ?
  ...
  setValue(value, flags = {}) {
    ...
    if (this.isInitApiCallNeeded(hasValue)) {
      ...
      const searchProperty = this.component.searchField || this.component.valueProperty || (this.component.data.noco_db_conditions&&this.component.data.noco_db_conditions.length>0&&this.component.data.noco_db_conditions[0].name.length>0);
  ...
  isInitApiCallNeeded(hasValue) {
    return this.component.lazyLoad &&
      ...
      hasValue &&
      this.shouldLoad &&
      this.visible && (this.component.searchField || this.component.valueProperty || (this.component.data.noco_db_conditions&&this.component.data.noco_db_conditions.length>0&&this.component.data.noco_db_conditions[0].name.length>0));
  }
