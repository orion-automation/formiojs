...
export default class DateTimeRangeComponent extends Input {
  ...
  get defaultValue() {
    ...
    if (!defaultValue||defaultValue.length===0) {
      ...
      if (this.component.defaultStartDate) {
        const parsedVal = FormioUtils.getDateSetting(this.component.defaultStartDate);
        if (parsedVal) {
          tmpVal.push(parsedVal.toISOString());
        }
      }
      if (this.component.defaultEndDate) {
        const parsedVal = FormioUtils.getDateSetting(this.component.defaultEndDate);
        if (parsedVal) {
          tmpVal.push(parsedVal.toISOString());
        }
      }
