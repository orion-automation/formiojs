...
export default class HtmlEditorComponent extends TextFieldComponent {
  ...
  static get builderInfo() {
    return {
      ...
      weight: 20,
      schema: HtmlEditorComponent.schema()
    };
  }
  ...
  get defaultSchema() {
    return HtmlEditorComponent.schema();
  }
          ...
          this.addCKE(element, settings, (newValue) => this.updateEditorValue(index, newValue))
            .then((editor) => {
              ...
              if (getBrowserInfo().ie) {
                ...
                // 支持ckeditor34.0.0以上写法
                if (isReadOnly) {
                  editor.enableReadOnlyMode(this.component.id);
                }
                else {
                  editor.disableReadOnlyMode(this.component.id);
                }
                // ckeditor34.0.0以下
  ...
  setConvertedValue(value, index) {
    if (value&&value.html){
      value=decodeURIComponent(escape(window.atob(value.html)));
    }
    if (!_.isString(value)) {
      value = '';
    }
  ...
  getConvertedValue(value) {
    return { html: window.btoa(unescape(encodeURIComponent(value))) };
  }
