...
export default class FileComponent extends Field {
  ...
  getValueAsString(value) {
    if (_.isArray(value)) {
      return _.map(value, 'fileName').join(', ');
    }

    return _.get(value, 'fileName', '');
  }
  ...
  render() {
    return super.render(this.renderTemplate('fileUDS', {
  ...
  upload(files) {
    ...
    if (this.component.storage && files && files.length) {
      ...
      Array.prototype.forEach.call(files, async (file) => {
        ...
        const fileWithSameNameUploaded = this.dataValue.some(fileStatus => fileStatus.fileName === file.name);
        const fileWithSameNameUploadedWithError = this.statuses.findIndex(fileStatus =>
          fileStatus.fileName === file.name
    ...
    fileService.downloadFile(fileInfo, options).then((file) => {
      if (file) {
        if (['base64', 'indexeddb'].includes(file.storage)) {
          download(file.url, file.fileName || file.name, file.type);
        } else {
